...
function RegistrationSuccesfull() {
  ...
  useEffect(() => {
    ...
    var params = {
      ...
      source: "API",
      body:
        "Thank you for registering with LearnBowl! We confirm your seat for the " +
        course +
        " workshop. Your roll no is: " +
        LBRollNumber +
        ".Other details will be sent shortly. Stay tuned!",
        ...
        .then(async (response) => {
          ...
          if (response.status === 200) {
            ...
            await Axios.get("/getworkshopdate", {
              ...
              .then((response) => {
                if (response.status === 200) {
                  let tempWorkShopDate = response.data[0].workshopdates;
                  let datesArray = tempWorkShopDate.split("-");
                  setWorkShopDate(datesArray[0]);
    ...
    <div className="registrationsuccessfull">
      ...
      <div className="registrationsuccessfull-section-container">
        <div className="registrationsuccessfull-section-1">
          <p>
            <span style={{ color: "#2D3092", fontWeight: "600" }}>
              Hey {name},
            </span>
          </p>
          <p>
            You have successfully registered for LearnBowl’s workshop on the{" "}
            <b>{course === "French" ? "French Language" : "Vedic Maths"}</b>.
            The details of the workshop will be sent via SMS on the registered
            number.
            <br />
            <br />
            We here at LearnBowl extend you a warm welcome and hope that you
            remain by our side in many more learning adventures ahead! See you
            on{" "}
            <span style={{ color: "#2D3092", fontWeight: "600" }}>
              {workshopDate}
            </span>
          ...
          <p>
            <span style={{ color: "#2D3092", fontWeight: "600" }}>
              Workskhop dates chosen:
            </span>{" "}
            <b>{workshopDate}</b>
          </p>
          <p>
            <span style={{ color: "#2D3092", fontWeight: "600" }}>Time:</span>{" "}
            <b>{workshopTime}</b>
          </p>
        ...
        <div className="registrationsuccessfull-section-2">
          <img
            src="/registrationsuccessfull.svg"
            alt="image"
            className="registrationsuccessfull-img"
          ></img>
        </div>
      </div>
    </div>
